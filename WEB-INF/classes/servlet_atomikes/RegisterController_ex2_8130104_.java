...
import java.util.*;
import exercise2_8130104.*;

public class RegisterController_ex2_8130104 extends HttpServlet {
	public void doPost (HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		User cUser = (User) session.getAttribute("ex2-user-object");
		...
		try{
			...
			out.println("				</div>");
			out.println("				<div id='navbar' class='navbar-collapse collapse'  id='navbarSupportedContent'>");
			out.println("					<ul class='nav navbar-nav'>");
			out.println("						<li><a href='../index_ex2_8130104.jsp'>Home</a></li>");
			out.println("						<li class='active' ><a href='../register_ex2_8130104.jsp'>Register</a></li>");
			out.println("						<li><a href='../about_ex2_8130104.jsp'>About</a></li>");
			out.println("                    </ul>");
			out.println("					<ul class='nav navbar-nav navbar-right'>");
			out.println("						<li class='dropdown'>");
			out.println("							<a class='dropdown-toggle' data-toggle='dropdown' href='#' aria-haspopup='true'><span class='glyphicon glyphicon-user'></span>");
			out.println("							<%= cName %><span class='caret'></span></a>");
			out.println("							<div class='dropdown-menu'>");
			out.println("							  <a class='dropdown-item' href='../logout_ex2_8130104.jsp'><span class='glyphicon glyphicon-log-out'></span>Logout</a>");
			out.println("							</div>");
			out.println("						</li>");
			out.println("					</ul>");
			out.println("				</div>");
			...
			if ((newInstance.isAmValid(am) == true)&&(newInstance.isFullnameValid(fullname) == true)&&(newInstance.isValidPhoneNumber(phone) == true)&&(newInstance.isValidGender(gender) == true)&&(newInstance.isValidEmailAddress(email) == true)&&(newInstance.checkDepartment(department) == true)){
				exercise2_8130104.DepartmentDAO dept = new exercise2_8130104.DepartmentDAO();
				...
				out.println("					<a href='../register_ex2_8130104.jsp' class='btn-lg btn-info' role='button'><span class='glyphicon glyphicon-chevron-left'></span>Back to form</a>");
				out.println("				</div>");
				out.println("		</div>");
				out.println("		</div>");
				out.println("		<!-- /container -->");
	...
	public void doGet (HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		User cUser = (User) session.getAttribute("ex2-user-object");
		if (session.getAttribute("ex2-user-object") == null){
			String redirect = "/index_ex2_8130104.jsp";
