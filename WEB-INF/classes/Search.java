...
public class Search extends HttpServlet {
	public void doPost 	(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		//END: Session Control
		//Search Process
		String search_entry = request.getParameter("search_entry");
		session.setAttribute("search_entry", search_entry);
		...
		String warning = null;
		try {
			products = DM.simpleSearch(search_entry);
		} catch (Exception e) {
		...
		try {
			...
			out.println("	<title>Search: "+ search_entry +" | OpenCommerce</title>");
			...
			out.println("					<h4>Search Results: <small>"+ search_entry +"</small></h4>");
			out.println("				</div>");
			...
			if (products.size() == 0) {
				...
				for (Product product : products) {
					...
					while (!alreadyDisplayed.contains(product.getId())) {
						alreadyDisplayed.add(product.getId());
						counter++;
						String avgRating;
						Category category =product.getCategory();
						try {
						Double avg = pdao.getAverageRatings(product);
						avgRating = Double.toString(avg);
						} catch (Exception e) {
							avgRating = "no ratings found";
						}
						if (counter == 1 && rowcheck > 0) {
							out.println("			<div class='row has-top-margin'>");
							out.println("				<div class='col-md-4'>");
							out.println("					<article class='article-post'>");
							out.println("						<a href='ViewProduct?prod_id="+product.getId()+"'>");
			...
			out.println("				</div>");
			out.println("			</section>");
			out.println("	<!-- END SECTION: Articles -->");
