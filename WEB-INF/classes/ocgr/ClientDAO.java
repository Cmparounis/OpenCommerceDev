...
public class ClientDAO {
	...
	*/
	...
	public HashMap<Product, Rating>  getRatingsByClient (Client client) throws Exception{
		Connection con = null;
		String sqlquery = "SELECT * FROM ocgr_ratings LEFT JOIN  ocgr_products ON ocgr_ratings.product_id = ocgr_products.product_id LEFT JOIN  ocgr_categories ON ocgr_categories.category_id = ocgr_products.category_id WHERE client_id = ? ;";
		DB db = new DB();
		...
		try {
			...
			while (rs.next()){
				...
				Rating currentRating = new Rating(client.getId(), currentProduct.getId(), rs.getDouble("rating"), rs.getString("comment") );
